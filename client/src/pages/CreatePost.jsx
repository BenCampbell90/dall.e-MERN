...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch('https://dall-e-frontend-rho.vercel.app/api/v1/post', {
          method: 'POST',
          headers: {
  ...
  const generateImg = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://dall-e-frontend-rho.vercel.app/api/v1/dalle', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
